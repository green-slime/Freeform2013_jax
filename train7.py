...
def train_using_LM():
    ...
    #print(f"cfg.rx={cfg.rx},cfg.ry={cfg.ry},width={img_dict['width']},height={img_dict['height']}")
    ...
    while ((not stop) and (k < max_iter)):
        ...
        if (k==5):
            rd.render(Pij, s, img_dict, cfg.prefix_name+"5_train7Test.png")
        elif (k==30):
            rd.render(Pij, s, img_dict, cfg.prefix_name+"30_train7Test.png")
        ...
        if (k > 1 and (abs(loss-last_loss) < 1e-16 or stop or k == max_iter-1)):
            ...
            if (using_varyweight_flag == True):
                ...
                # update parameters
                v = 2
                #J = calculate_Jacobi(surface_dict, Pij, indices, img_dict, using_varyweight_flag)
                epsilon_p = - \
...
if __name__ == "__main__":
    ...
    print("=============================================\n Now begin OT Optimization")
    # train()
    Pij, surface, img_dict = train_using_LM()
    rd.render(Pij, surface, img_dict, cfg.createPicName("train7_OT"))
    # surface only depends on M and N : s=BSurface.BSurface(cfg.M,cfg.N)
    uf.saveToDict({"Pij": Pij, "M": cfg.M, "N": cfg.N},cfg.createDictName("train7_OT"))
    #uf.writeToObj(surface, Pij, cfg.objname)
    
    print("=============================================\n Now begin renderLoss Optimization")
    Pij,surface,img_dict = train8.train_using_LM(Pij,surface,img_dict)
    rd.render(Pij, surface, img_dict, cfg.createPicName("train7_renderLoss"))

    # surface only depends on M and N : s=BSurface.BSurface(cfg.M,cfg.N)
    uf.saveToDict({"Pij": Pij, "M": cfg.M, "N": cfg.N},cfg.createDictName("train7_renderLoss"))