...
def calculate_A_and_g(surface_dict, Pij, indices,img_dict, using_varyweight_flag):
    ...
    for i in range(forNum):
        indices_need=indices[i*MaxEqutionNum:min((i+1)*MaxEqutionNum,cfg.totalSampleNum),:]
...
def train_using_LM(Pij,s,img_dict):
    ...
    while ((not stop) and (k < max_iter)):
        ...
        if (k==5):
            rd.render(Pij, s, img_dict, cfg.prefix_name+"5_trainOT.png")
        elif (k==30):
            rd.render(Pij, s, img_dict, cfg.prefix_name+"30_train5OT.png")
        ...
        if (k > 1 and (abs(loss-last_loss) < 1e-16 or stop or k == max_iter-1)):
            ...
            if (using_varyweight_flag == True):
                ...
                rd.render(Pij, s, img_dict, cfg.createPicName("train5_only_OT_pre"))
                logfile.write(f"Now using_varyweight_flag=False\n")
                using_varyweight_flag = False
                # update parameters
                v = 2
                epsilon_p = - \
                    loss_func(surface_dict, Pij, indices, img_dict,
                              using_varyweight_flag, True)
                A,g=calculate_A_and_g(surface_dict, Pij, indices,img_dict, using_varyweight_flag)
                mu = tao*jnp.max(jnp.diag(A))
                stop = False
...
import train_initRect as tir
import train_onlyRenderloss as tor
if __name__ == "__main__":
    ...
    Pij, surface, img_dict = tir.train_using_LM(Pij, surface, img_dict)
    rd.render(Pij, surface, img_dict, cfg.createPicName("train_Init2"))
    uf.saveToDict({"Pij": Pij, "M": cfg.M, "N": cfg.N},cfg.createDictName("train_Init2"))
    '''
    Pij, surface, img_dict = uf.integrateReadFromDict(cfg.createDictName("train_Init"))
    
    print("=============================================\n Then use OT Optimization")
    # train()
    Pij, surface, img_dict = train_using_LM(Pij,surface,img_dict)
    rd.render(Pij, surface, img_dict, cfg.createPicName("train_OT_afterInit"))
    uf.saveToDict({"Pij": Pij, "M": cfg.M, "N": cfg.N},cfg.createDictName("train_OT_afterInit"))
    
    print("=============================================\n Finally use renderloss Optimization")
    # train()
    Pij, surface, img_dict = tor.train_using_LM(Pij,surface,img_dict)
    rd.render(Pij, surface, img_dict, cfg.createPicName("train_renderloss_afterOT"))
    # surface only depends on M and N : s=BSurface.BSurface(cfg.M,cfg.N)
    uf.saveToDict({"Pij": Pij, "M": cfg.M, "N": cfg.N},cfg.createDictName("train_renderloss_afterOT"))
