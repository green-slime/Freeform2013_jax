...
def Jacobi_for_one_pos(i,j,Pij,avg_len,renderMeshdict,target_intensity):
    ...
    res_temp=(rd.render_loss(P_temp1,renderMeshdict,target_intensity)-rd.render_loss(P_temp2,renderMeshdict,target_intensity))/(2*avg_len)
    return res_temp
...
def update_rho_p_f(Pij,delta_p,mu,g,epsilon_p,renderMeshdict,target_intensity):
    ...
    f_new=rd.render_loss(p_new,renderMeshdict,target_intensity)              
...
def if_positive_rho(epsilon_p,f_new,rho,mu,p_new,epsilon_g_norm,epsilon_relative,Pij,renderMeshdict,target_intensity):
    ...
    print(A_new.shape)
    epsilon_p_new=-rd.render_loss(Pij,renderMeshdict,target_intensity)
...
def solve_using_LM(Pij,surface,imgdict):
    ...
    k=0;v=2;J=calculate_Jacobi(Pij,renderMeshdict,target_intensity);epsilon_p=-rd.render_loss(Pij,renderMeshdict,target_intensity)
