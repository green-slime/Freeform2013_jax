...
def train_using_LM():
    ...
    while ((not stop) and (k < max_iter)):
        ...
        if (k==5):
            rd.render(Pij, s, img_dict, cfg.prefix_name+"5_train5Test.png")
        elif (k==30):
            rd.render(Pij, s, img_dict, cfg.prefix_name+"30_train5Test.png")
...
if __name__ == "__main__":
    ...
    rd.render(Pij, surface, img_dict, cfg.createPicName("train5_only_OT"))

    # surface only depends on M and N : s=BSurface.BSurface(cfg.M,cfg.N)
    uf.saveToDict({"Pij": Pij, "M": cfg.M, "N": cfg.N},cfg.createDictName("train5_only_OT"))
