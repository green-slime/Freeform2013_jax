...
def E_test(tx,ty):
    ...
    '''
    weight=10./((jnp.sqrt(2)-1)*r0)**2 # notice that r<=(sqrt(2)-1)*r0
    r=boundary_test(tx,ty)
...
def rect_boundary(tx,ty):
    w=cfg.half_width
    h=cfg.half_height
    condlist=jnp.array([(tx<-w)&(ty<-h),(tx<-w)&(ty>=-h)&(ty<=h),(tx<-w)&(ty>h),(tx>=-w)&(tx<=w)&(ty>h),(tx>w)&(ty>h),(tx>w)&(ty>=-h)&(ty<=h),(tx>w)&(ty<-h),(tx>=-w)&(tx<=w)&(ty<-h),(tx>=-w)&(tx<=w)&(ty>=-h)&(ty<=h)])
    #print("condlist:",condlist)
    # 3   4    5
    # 2   9    6
    # 1   8    7
    inner_array=jnp.array([jnp.abs(tx+w),jnp.abs(ty+h),jnp.abs(tx-w),jnp.abs(ty-h)])
    choicelist=jnp.array([jnp.sqrt((tx+w)**2+(ty+h)**2),jnp.abs(tx+w),jnp.sqrt((tx+w)**2+(ty-h)**2),jnp.abs(ty-h),jnp.sqrt((tx-w)**2+(ty-h)**2),jnp.abs(tx-w),jnp.sqrt((tx-w)**2+(ty+h)**2),jnp.abs(ty+h),jnp.min(inner_array)])
...
def img_E(tx,ty,img_dict):
    w=cfg.half_width
    h=cfg.half_height
    S_pixel=img_dict["S_pixel"];min_intensity=img_dict["min_intensity"]
    width=img_dict["width"];height=img_dict["height"]
