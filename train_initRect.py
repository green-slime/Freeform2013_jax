...
def loss_func_for_one_pos(i, j, bool, gNui3, gNvi3, Pij, gdNui3, gdNvi3, gddNui3, gddNvi3, ni, no, a, tz, img_dict):
    ...
    x, y, curIndex, z, zx, zy, zxx, zxy, zyy, b, Ox, Oy, Oz, tx, ty = cost_func.args_calculation(
        i, j, gNui3, gNvi3, Pij, gdNui3, gdNvi3, gddNui3, gddNvi3, ni, no,cols=cfg.Init_sample,rows=cfg.Init_sample)
    # print("(j,i)=,",j,i,"z=",z,"tx,ty=",tx,ty)
    #res = lax.cond(bool, lambda x: cost_func.inner_cost_func_forInit(no, ni, a, b, z, zx, zy, tx, ty,tz, zxx, zyy, zxy, img_dict), lambda x: cost_func.boundary_cost_func(tx, ty)*weight, 0)
...
def loss_func(dict, Pij, indices, img_dict, using_varyweight_flag, flag_printout=False) -> jnp.ndarray:
    # loss function
    gNui3 = dict["gNui3_forInit"]
    gNvi3 = dict["gNvi3_forInit"]
    gdNui3 = dict["gdNui3_forInit"]
    gdNvi3 = dict["gdNvi3_forInit"]
    gddNui3 = dict["gddNui3_forInit"]
    gddNvi3 = dict["gddNvi3_forInit"]
    no = cfg.no
    ni = cfg.ni
    a = cfg.a
    tz = cfg.tz
...
def if_positive_rho(epsilon_p, f_new, rho, mu, p_new, surface_dict, indices, epsilon_g_norm, epsilon_relative, img_dict, using_varyweight_flag):
    ...
    p = p_new
    A_new,g_new=calculate_A_and_g(surface_dict, p, indices,img_dict, using_varyweight_flag)
...
def calculate_A_and_g(surface_dict, Pij, indices,img_dict, using_varyweight_flag):
    ...
    process at one time.
    """
    MaxEqutionNum=cfg.maxEqNum
    forNum = int(cfg.totalInitSampleNum/MaxEqutionNum)+1
    A=jnp.zeros((cfg.totalBasisNum,cfg.totalBasisNum))
    g=jnp.zeros(cfg.totalBasisNum)
    for i in range(forNum):
        indices_need = indices[i*MaxEqutionNum:min(
            (i+1)*MaxEqutionNum, cfg.totalInitSampleNum), :]
        #print(indices_need.shape)
        epsilon_p = -loss_func(surface_dict, Pij, indices_need,
                           img_dict, using_varyweight_flag)
        J=calculate_Jacobi(surface_dict, Pij, indices_need,img_dict, using_varyweight_flag)
...
def train_using_LM(Pij,s,img_dict):
    ...
    #                     img_dict, using_varyweight_flag)
    epsilon_p = -loss_func(surface_dict, Pij, indices,img_dict, using_varyweight_flag, True)
    epsilon_g_norm = 1e-8
    ...
    A,g=calculate_A_and_g(surface_dict, Pij, indices,img_dict, using_varyweight_flag)
    tao = 1e-6
    ...
    while ((not stop) and (k < max_iter)):
        ...
        if (k==5):
            rd.render(Pij, s, img_dict, cfg.prefix_name+"Init_5.png",rm=int(cfg.rm/2),rn=int(cfg.rn/2),colored_picPath=cfg.prefix_name+"Init_5_colored.png")
        elif (k==30):
            rd.render(Pij, s, img_dict, cfg.prefix_name+"Init_30.png",rm=int(cfg.rm/2),rn=int(cfg.rn/2),colored_picPath=cfg.prefix_name+"Init_30_colored.png")
            
        if (k > 1 and (((abs(loss-last_loss) < 1e-3) and last_loss/loss<3) or stop or k == max_iter-1)):
            # if loss not decay very rapidly, end it.
            str = ""
...
if __name__ == "__main__":
    ...
    print("=============================================\n Only use OT Optimization for Initialization")
    # train()
    Pij, surface, img_dict = train_using_LM(Pij,surface,img_dict)
    rd.render(Pij, surface, img_dict, cfg.createPicName("train_Init"))

    # surface only depends on M and N : s=BSurface.BSurface(cfg.M,cfg.N)
    uf.saveToDict({"Pij": Pij, "M": cfg.M, "N": cfg.N},cfg.createDictName("train_Init"))
