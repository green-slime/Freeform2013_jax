...
def train_using_LM(Pij,s,img_dict):
    ...
    while ((not stop) and (k < max_iter)):
        ...
        if (k == 5):
            rd.render(Pij, s, img_dict, cfg.prefix_name+"5_train8Test.png")
        elif (k == 30):
            rd.render(Pij, s, img_dict, cfg.prefix_name+"30_train8Test.png")
...
if __name__ == "__main__":
    ...
    rd.render(Pij, surface, img_dict, cfg.createPicName("train8_only_renderloss"))

    # surface only depends on M and N : s=BSurface.BSurface(cfg.M,cfg.N)
    uf.saveToDict({"Pij": Pij, "M": cfg.M, "N": cfg.N}, cfg.createDictName("train8_only_renderloss"))